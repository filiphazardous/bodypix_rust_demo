...
pub fn mask_to_image(mask: &Vec<Vec<u8>>) -> DynamicImage {
    let width = mask.len();
    let height = mask[0].len();

    let output_img = ImageBuffer::from_fn(width as u32, height as u32, |x, y| {
        let shade_val: u8 = mask[x as usize][y as usize] * 255;
...
pub fn create_silhouette(mask: &Vec<Vec<u8>>, orig: &DynamicImage) -> DynamicImage {
    let width = orig.width();
    let height = orig.height();
    let black_pixel = image::Rgb([0, 0, 0]);
    let silhouette_img = ImageBuffer::from_fn(width, height, |x, y| {
        if mask[x as usize][y as usize] > 0 {
...
pub fn create_cutout(mask: &Vec<Vec<u8>>, orig: &DynamicImage) -> DynamicImage {
    let width = orig.width();
    let height = orig.height();
    let black_pixel = image::Rgb([0, 0, 0]);
    let silhouette_img = ImageBuffer::from_fn(width, height, |x, y| {
